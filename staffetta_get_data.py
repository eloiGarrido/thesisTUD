...
class LogConverter(object):
                ...
                if split_packet[l] != []:
                    if split_packet[l][idx] not in splited:
                        ...
                        for j in range(0, len(split_packet[l])):
                            src = splited[idx][0][3]
                            seq = splited[idx][0][2]
                            if split_packet[l][j] not in splited[idx] and split_packet[l][j][3] == src and \
                                            split_packet[l][j][2] == seq:
        ...
        for idx in range(0, len(orig_packet)):
            ...
            if orig_packet[idx] != []:
                if sink_packet[idx] == []:
                    ...
                    pkt_delay.append(
                        {'src': int(orig_t[3]), 'seq': int(orig_t[2]), 'delay': (int(sink_t[4]) - int(orig_t[4]))})
        ...
        for len_orig_pkt in range(0, len(orig_packet)):
            ...
            for sink_idx in range(0, len(self.nodes[0]['pkt'])):
                if self.nodes[0]['pkt'] != []:
                    ...
                    if int(packet_sink_t[3]) == packet_orig_t[0] and int(packet_sink_t[2]) == packet_orig_t[
                        2]:  # Same origin and sequence code
                        sink_packet.append([])
    ...
    def print_drop_ratio(self, pkt_delay):
        ...
        plt.annotate(str(sum(drop_pkt) / sum(total_pkt)),
                     xy=(self.number_of_nodes - 2, (sum(drop_pkt) / sum(total_pkt)) + 0.05))
        # Output data
    ...
    def print_dc(self):
        ...
        plt.annotate(str(float(sum(avg_dc_array)) / float(len(avg_dc_array))),
                     xy=(self.number_of_nodes - 2, (float(sum(avg_dc_array)) / float(len(avg_dc_array)) + 0.5)))
...
if __name__ == '__main__':
    ...
    for (dirpath, dirnames, filenames) in walk("/media/egarrido/data/simulations/8_7_16/"):
        f.extend(filenames)
        break
    for file in f:
        name = file.split('_')
